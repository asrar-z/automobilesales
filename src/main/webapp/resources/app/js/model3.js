...
//asrar
	...
	$.each(data.makes,function(i, make){
					...
					$.each(data2.vehicles,function(i,vehicle){
						temp += '<tr><td>'+ vehicle.name +'</td><td>'+vehicle.cc +'</td><td>'+vehicle.torque +'</td><td>'+vehicle.speed +'</td><td>'+ vehicle.cost+'</td><td>'+vehicle.sold+'</td><td>'+vehicle.quantity +'</td><td><img src="data:image/jpeg;base64,'+vehicle.img +'" width="195" height="115"></img></td></tr>'
								//console.log(temp)
						})
					...
					$.each(outer, function(i,tbody){
						$(tbody).append($('<tr data-toggle="collapse" data-target=#'+make.name+' class="accordion-toggle">\
							...
							<thead> \
								<tr><th>Model</th><th>Engine Displacement</th><th>Torque </th><th>Speed</th><th>Cost</th><th>Quantity Sold</th><th>Quantity remaining</th><th>Image</th></tr>\
							</thead>\
							 <tbody>'+ temp+'</tbody>\
							</table>\
							</div>\
